	...
	public void createHeadMask(Tensor lens,Tensor mask,int number,int maxLen,int headNum) {
		...
		try {
	        ...
	        kernelParameters = Pointer.to(
	        		Pointer.to(new long[]{mask.dataLength}),
	        		Pointer.to(lens.getGpuData()),
	        		Pointer.to(mask.getGpuData()),
	                Pointer.to(new int[]{number}),
	                Pointer.to(new int[]{maxLen}),
	                Pointer.to(new int[]{headNum})
	            );

			cuLaunchKernel(function,
		            this.CAFFE_GET_BLOCKS(mask.dataLength),  1, 1,      // Grid dimension
	...
	public void createHeadUnMask(Tensor lens,Tensor mask,int number,int max_label_len,int max_feat_len,int headNum) {
		...
		try {
	        ...
	        kernelParameters = Pointer.to(
	        		...
	        		Pointer.to(mask.getGpuData()),
	                Pointer.to(new int[]{number}),
	                Pointer.to(new int[]{max_label_len}),
	                Pointer.to(new int[]{max_feat_len}),
	                Pointer.to(new int[]{headNum})
	            );

			cuLaunchKernel(unmask_function,
		            this.CAFFE_GET_BLOCKS(mask.dataLength),  1, 1,      // Grid dimension
		            CAFFE_CUDA_NUM_THREADS, 1, 1,      // Block dimension
		            0, null,               // Shared memory size and stream
		            kernelParameters, null // Kernel- and extra parameters
